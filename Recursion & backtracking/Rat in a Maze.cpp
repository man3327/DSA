...
class Solution {
  ...
  void recurse(vector<string> &vec,string &s,vector<vector<int>>&maze,int row,int col){
      if(row>=maze.size() || col>=maze.size()) return;
      ...
      vis[row][col]=true;
       s.push_back('D');
      recurse(vec,s,maze,row+1,col);
      s.pop_back();
      s.push_back('L');
      recurse(vec,s,maze,row,col-1);
      s.pop_back();
      s.push_back('R');
      recurse(vec,s,maze,row,col+1);
      s.pop_back();
